...
@Component({
	selector: 'app-act-applicant-filter',
	standalone: true,
	styleUrl: 'applicant-filter.component.scss',
...
export class ActApplicantsFilterComponent
	...
	implements OnInit
{
	...
	public override getList$(query: string): Observable<IDictionaryItemDto[]> {
		return this.filterApiService.getApplicantUsers(query).pipe(
			map((value) => {
				return value.items;
			})
		);
	...
	public override searchActive$(
		...
		return this.filterApiService.getApplicantUsers('', ids).pipe(
			map((value) => {
				return value.items;
			})
		);
