...
export class ClientProposalsCardComponent {
	protected waitingForLoading$: BehaviorSubject<boolean> =
	...
	protected defaultStateTable$: BehaviorSubject<boolean> =
		new BehaviorSubject<boolean>(false);

	protected expandedPopoverCheckColumn$: BehaviorSubject<boolean> =
		new BehaviorSubject<boolean>(false);
	...
	public vgpQueryControl: FormControl<string | null> = new FormControl<
		string | null
	>(null);

	public tgQueryControl: FormControl<string | null> = new FormControl<
		string | null
	>(null);

	public tpgQueryControl: FormControl<string | null> = new FormControl<
		string | null
	>(null);

	protected offersItems$: BehaviorSubject<IClientOffersDto[]> =
		new BehaviorSubject<IClientOffersDto[]>([]);

	protected readonly form: FormGroup<IClientProposalsCriteriaForm> =
		this.buildForm();

	private readonly clientId$: BehaviorSubject<number | null> =
	...
	constructor(
		...
		this.productionOptionsVgp$ = this.vgpQueryControl.valueChanges.pipe(
			filterTruthy(),
			switchMap((val) => {
				return this.clientProposalsFacadeService
					.getProduction(val)
					.pipe(
						untilDestroyed(this),
						filterTruthy(),
	...
	private buildForm(): FormGroup<IClientProposalsCriteriaForm> {
		return new FormGroup<IClientProposalsCriteriaForm>({
			...
			tgIds: new FormControl<number[] | null>({
				value: null,
				disabled: false,
			}),
			TprFlags: new FormControl<number[] | null>({
				value: null,
				disabled: false,
			}),
		});
	...
	protected submit() {
		...
		if (
			this.form.valid &&
			!this.clientProposalsFacadeService.blockForProposalSubject$.value
		) {
	...
	protected saveFiles() {
		if (
			this.checkListStateService.checkFiles$.value.length &&
			!this.waitingForLoading$.value
		) {
			this.clientProposalsFacadeService
				.saveInCloud(
